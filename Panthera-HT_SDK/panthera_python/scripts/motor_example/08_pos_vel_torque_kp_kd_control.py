...
if __name__ == "__main__":
    ...
    while True:
        ...
        for motor in motors:
            # 设置目标位置为正弦波变化
            t = time.time()
            pos = 1.0 * math.pi * math.sin(2.0 * math.pi * 0.1 * t)  # 0.1 Hz, 幅值1.0 rad
            vel = 0.2 * math.pi * math.pi * math.cos(2.0 * math.pi * 0.1 * t)  # 目标速度为位置导数
            motor.pos_vel_tqe_kp_kd(pos, vel, 0.0, 2, 0.5)
